...
static std::vector<TextureDescription> GetTextureDescriptions(AVPixelFormat fmt)
{
    switch (fmt) {
...
std::vector<AVPixelFormat> ImageRenderItem::formats()
{
    // clang-format off
    return {
...
void ImageRenderItem::create(QRhi *rhi, QRhiRenderTarget *rt)
{
    ...
    auto params = GetTextureDescriptions(fmt_.pix_fmt);
    planes_.clear();
    ...
    const auto name =
        QString(":/src/resources/shaders/") + ShaderNameForPixelFormat(fmt_.pix_fmt) + ".frag.qsb";
...
void ImageRenderItem::upload(QRhiResourceUpdateBatch *rub, float scale_x, float scale_y)
{
    ...
    const auto params                      = GetTextureDescriptions(fmt_.pix_fmt);
