...
void ImageWindow::show(bool visible)
{
    ...
    case WindowStatus::CREATED:
        ...
        resize(getShadowSize(pixmap_.size()));
...
{
    switch (type) {
    case Modified::ROTATED: pixmap_ = pixmap_.transformed(QTransform().rotate(+1 * 90.0)); break;

    case Modified::ANTI_ROTATED: pixmap_ = pixmap_.transformed(QTransform().rotate(-1 * 90.0)); break;

    case Modified::FLIPPED_V: pixmap_ = pixmap_.transformed(QTransform().scale(1, -1)); break;

    case Modified::FLIPPED_H: pixmap_ = pixmap_.transformed(QTransform().scale(-1, 1)); break;
    ...
    case Modified::GRAY:
        if (pixmap_.hasAlpha()) {
            ...
            pixmap_ =
                QPixmap::fromImage(pixmap_.toImage().convertToFormat(QImage::Format::Format_Grayscale8));
        }
        break;
    ...
    auto geometry = getShadowGeometry(pixmap_.size() * scale_);
#ifdef _WIN32
    ...
    QWidget::update(rect().adjusted(shadow_r_, shadow_r_, -shadow_r_, -shadow_r_));
}
...
void ImageWindow::paintEvent(QPaintEvent *)
{
    ...
    if (bg_ != Qt::transparent && pixmap_.hasAlpha()) {
        painter.fillRect(shadow_r_, shadow_r_, width() - 2 * shadow_r_, height() - 2 * shadow_r_, bg_);
    }

    if (thumbnail_) {
        auto center = pixmap_.rect().center();
        painter.drawPixmap(QRect{ shadow_r_, shadow_r_, 125, 125 }, pixmap_,
                           { center.x() - THUMBNAIL_WIDTH_ / 2, center.y() - THUMBNAIL_WIDTH_ / 2,
                             THUMBNAIL_WIDTH_, THUMBNAIL_WIDTH_ });
    }
    else {
        painter.setRenderHints(QPainter::SmoothPixmapTransform | QPainter::Antialiasing);
        painter.drawPixmap(rect().adjusted(shadow_r_, shadow_r_, -shadow_r_, -shadow_r_), pixmap_);
    }
}
...
void ImageWindow::initContextMenu()
{
    ...
    addAction(context_menu_->addAction(tr("Copy image"),    [this]() { QApplication::clipboard()->setPixmap(image()); }, QKeySequence(Qt::CTRL | Qt::Key_C)));
    ...
    addAction(context_menu_->addAction(tr("Rotate -90"),    [this]() { update(Modified::ANTI_ROTATED); },   QKeySequence(Qt::CTRL | Qt::Key_R)));
    addAction(context_menu_->addAction(tr("H Flip"),        [this]() { update(Modified::FLIPPED_H); },      QKeySequence(Qt::Key_H)));
    addAction(context_menu_->addAction(tr("V Flip"),        [this]() { update(Modified::FLIPPED_V); },      QKeySequence(Qt::Key_V)));
...
void ImageWindow::dropEvent(QDropEvent *event)
{
    ...
    scale_ = 1.0;
    image(QPixmap(path));
    setGeometry(getShadowGeometry(pixmap_.size()));
...
void ImageWindow::dragEnterEvent(QDragEnterEvent *event)
{
    ...
    if (mimedata->hasUrls() && QString("jpg;png;jpeg;JPG;PNG;JPEG;bmp;BMP;ico;ICO;gif;GIF;svg;SVG")
                                   .contains(QFileInfo(mimedata->urls()[0].fileName()).suffix()))
        event->acceptProposedAction();
}
...
{
    // clang-format off
    connect(new QShortcut(Qt::Key_Escape, this), &QShortcut::activated, [this]() { invisible(); });
